...
[STUDY, EEG]        = std_precomp(STUDY, ALLEEG, {}, 'savetrials','on','interp','on','recompute','on',...
    'erp','off', 'spec','off', 'ersp','on','itc','off');
eeglab redraw
...
limo_display_results(3,'ess_1.mat',pwd,0.05,2,fullfile(pwd,'LIMO.mat'),0,...
    'channels',49,'restrict','Time','dimvalue',5); % time course plot at 5Hz
saveas(gcf, 'contrast_timecourse.fig'); close(gcf)
limo_display_results(3,'ess_1.mat',pwd,0.05,2,fullfile(pwd,'LIMO.mat'),0,...
    'channels',49,'restrict','Frequency','dimvalue',180); % spectrum at 180ms
...
limo_eeg(5,pwd) % channel*time imagesc for both effects and contrast
limo_display_results(3,'ess_1.mat',pwd,0.05,2,fullfile(pwd,'LIMO.mat'),0,...
    'channels',49,'restrict','time','dimvalue',5); % course plot
saveas(gcf, 'contrast_5Hz_timecourse.fig'); close(gcf)
...
% ANOVA was computed - as such this is the right measure
...
limo_add_plots('channel',49,'restrict','Time','dimvalue',5,fullfile(fileparts(pwd),'LIMO.mat'),'variable',[1:18],...
...
[~,~,contrast.LIMO_files] = limo_get_files([],[],[],...
    ...
    'LIMO_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt'));
...
limo_random_select('one sample t-test',chanlocs,'LIMOfiles',...
    fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'con_4_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt'),...
...
% the long way to do this is loading the con files (stored in Yr but this works too)
Confiles = fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'con_4_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt');
limo_central_tendency_and_ci(Confiles, 2, chanlocs, 'Weighted mean', 'Trimmed Mean', [],fullfile(pwd,'con_avg.mat'));
cd ..
...
LIMOPath = limo_random_select('Repeated Measures ANOVA',chanlocs,'LIMOfiles',...
    fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'Beta_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt'),...
...
chanlocs = [STUDY.filepath filesep 'limo_gp_level_chanlocs.mat'];
files = {fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'con_5_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt'), ...
    fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'con_6_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt')};
...
Files{1} = fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'Beta_files_Gp1_ANOVA_Faces_GLM_Channels_Time-Frequency_WLS.txt');  
Files{2} = fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'Beta_files_Gp2_ANOVA_Faces_GLM_Channels_Time-Frequency_WLS.txt');  
...
limo_random_select('regression',chanlocs,'LIMOfiles',...  
    fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'con_1_files_FaceRepAll_GLM_Channels_Time-Frequency_WLS.txt'),...
...
limo_display_results(3,'Covariate_effect_1.mat',pwd,0.05,1,...
    fullfile(pwd,'LIMO.mat'),0,'channels',56,'plot3type','Adjusted','restrict','time','dimvalue',5); % course plot
saveas(gcf, 'contrast_timecourse.fig'); close(gcf)
...
% note 'splitreg' is 'on'
STUDY  = pop_limo(STUDY, ALLEEG, 'method','WLS','measure','dattimef',...
    'timelim',[-50 650],'freqlim',[3 45],'erase','on','splitreg','on','interaction','off');
...
limo_random_select('Repeated Measures ANOVA',chanlocs,'LIMOfiles',...
    fullfile(STUDY.filepath,['LIMO_' STUDY.filename(1:end-6)],'Beta_files_Face_time_GLM_Channels_Time-Frequency_WLS.txt'), ...
