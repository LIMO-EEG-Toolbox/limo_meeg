...
EEG = pop_clean_rawdata( EEG,'FlatlineCriterion',5,'ChannelCriterion',0.8,...
    ...
    'BurstCriterion',40,'WindowCriterion',0.25,'BurstRejection','off',...
...
% 2nd level analysis
mkdir([studypath filesep derivatives' filesep '2-ways-ANOVA'])
cd([studypath filesep derivatives' filesep '2-ways-ANOVA'])
...
chan_loc = [STUDY.filepath filesep 'limo_gp_level_chanlocs.mat'];
...
limo_central_tendency_and_ci(Files, parameters, chan_loc, 'Weighted mean', 'Mean', [],savename1) 
parameters = [4 5 6];
savename2  = [pwd filesep 'srambled_faces.mat'];
limo_central_tendency_and_ci(Files, parameters, chan_loc, 'Weighted mean', 'Mean', [],savename2) 
parameters = [7 8 9];
savename3  = [pwd filesep 'unfamiliar_faces.mat'];
limo_central_tendency_and_ci(Files, parameters, chan_loc, 'Weighted mean', 'Mean', [],savename3) 
...
STUDY = std_makedesign(STUDY, ALLEEG, 2, 'name','Face_time','delfiles','off','defaultdesign','off',...
    ...
    'variable2','time_dist','values1',SOMEVARIABLE,'vartype1','continuous',...
...
% 2nd level analysis
mkdir([studypath filesep derivatives' filesep '1-way-ANOVA'])
cd([studypath filesep derivatives' filesep '1-way-ANOVA'])
limo_random_select('Repeated Measures ANOVA',chanlocs,'LIMOfiles',...
    {[studypath filesep 'derivatives' filesep 'LIMO_Face_detection' filesep 'Beta_files_Face_time_GLM_Channels_Time_WLS.txt']},...
...
% add contrast
load('LIMO.mat')
LIMO.contrast{3}.C = [1 -05 -05 0];             % set contrast, note the last column 0 for constant`
LIMO.contrast{3}.V = 'F';                       % always F test for repeated measure ANOVA`
limo_contrast([pwd filesep 'Yr.mat'], LIMO, 3); % run the contrast
limo_contrast([pwd filesep 'Yr.mat'], LIMO, 4)  % apply bootstrap

% make topoplots
LIMO  = load('LIMO.mat'); LIMO = LIMO.LIMO;
stats = load('ess_2.mat');
stats = stats.(cell2mat(fieldnames(stats)));
